...
const donationOpportunities = [
  ...
  },
  {
    ...
    description: 'All proceeds go towards new medical supplies for the hospital.'
  },
  {
    id: 3,
    ...
    description: 'All proceeds go towards the wildlife conservation force of Dallas Park.'
  },
  {
    id: 4,
    dona_type: 'library',
  ...
  },
  {
    ...
    city_state: 'Richardson, TX',
    description: 'All proceeds go to animal shelter supplies for dogs, cats, bunnies.'
  },
  {
    id: 7,
  ...
  },
  {
    ...
    city_state: 'Richardson, TX',
    description: 'Infrastructure project #1: New school building on 15th street.'
  },
  ...
  const [distance, setDistance] = useState('');
   ...
   const handleDonationChange = (event) => {
    if (event.target.checked) {
      ...
      setDonations(donations.filter(donation => donation !== event.target.value));
    }
  };
      ...
      {/* Donation filter */}
        ...
        {donaTypes.map(donation => (
            <div key={donation}>
              ...
              />
              <label htmlFor={donation}>{" " + donation[0].toUpperCase() + donation.substring(1)}</label>
            </div>
          ))}
        </label><br></br>
...
const Donor = () => {
  const[filteredOpportunities, setFilteredOpportunities] = useState(donationOpportunities)

  const filterOpportunities = (filters) => {
    const filtered = donationOpportunities.filter(opportunity => {
      const donationMatch = !filters.donations.length || filters.donations.includes(opportunity.dona_type);
